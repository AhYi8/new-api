...
- 本地 Docker 启动统一使用 `scripts/start-local.ps1`，由脚本完成环境检查、镜像构建、容器替换和 `/api/status` 健康验证；禁止用缺少等价安全检查的临时命令替代。
- 根目录 `.env` 仅用于本地运行，必须同时被 `.gitignore` 和 `.dockerignore` 忽略；禁止将其中任何连接信息发送到 Docker 构建上下文、写入镜像层或提交到 Git。
- 本地实例必须连接独立的开发数据库和开发 Redis，禁止连接生产环境；应用启动会自动执行数据库迁移，并可能运行缓存同步和后台任务。
- 修改 `.env` 后必须重新执行启动脚本以重建容器，不能只重启旧容器，因为 Docker 不会自动刷新既有容器的环境变量。
