...
- Windows 本地启动统一使用 `scripts/start-local.ps1`，默认监听 `13000` 端口；脚本负责 Go、Bun、Git 和 `.env` 检查、停止脚本管理的旧进程、前端构建、Go 程序编译、进程重启及 `/api/status` 健康验证。本地开发启动禁止依赖 Docker，也禁止用缺少等价安全检查的临时命令替代。
- 根目录 `.env` 仅用于本地运行，必须同时被 `.gitignore` 和 `.dockerignore` 忽略；禁止提交其中的连接信息，或将其写入前端产物和可执行文件。
- 本地实例必须连接独立的开发数据库和开发 Redis，禁止连接生产环境；未配置 `SQL_DSN` 时使用本地 SQLite，未配置 `REDIS_CONN_STRING` 时禁用 Redis。应用启动会自动执行数据库迁移，并可能运行缓存同步和后台任务。
- 修改 `.env` 后必须重新执行启动脚本以替换旧进程，确保新进程重新加载环境变量。
- 脚本将可执行文件、PID 和控制台日志保存在已忽略的 `.local-tests/start-local/`；`-SkipBuild` 仅用于复用已经存在且确认有效的前端产物和本地可执行文件。
- 默认启动命令如下；首次连接远程开发库时，迁移和缓存初始化可能需要数分钟，必须以状态接口返回 HTTP 200 作为启动成功依据。
